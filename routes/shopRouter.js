...
shopRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Entity.find({ category: { $elemMatch: { $eq: 'isShop' } } })
  ...
  .put(
    ...
    (req, res) => {
      ...
      res
        .status(403)
        .end(
          `${req.method} opterations are not supported on ./shop path name.`
        );
    }
  )
  .delete(
  ...
  .post(
    ...
    (req, res) => {
      ...
      res
        .status(403)
        .end(
          `${req.method} operations are not supported on ./shop path name with a parameter of /${req.params.entity_id}.`
        );
    }
  )
  .put(
    ...
    (req, res, next) => {
      Entity.findByIdAndUpdate(
        ...
        .then((entity) => {
          ...
          //   .status(403)
          //   .end(
          //     `${req.method} opterations are not supported on ./shop path name with a parameter of /${req.params.entity_id}.`
          //   );
        })
        .catch((err) => next(err));
    }
  ...
  .put(
    ...
    (req, res) => {
      res
        .status(403)
        .end(
          `${req.method} opteration not supported on /shop/${req.params.entity_id}/comments`
        );
    }
  )
  .delete(
  ...
  .post(
    ...
    (req, res) => {
      res
        .status(403)
        .end(
          `${req.method} opteration not supported on /shop/${req.params.entity_id}/comments/${req.params.comment_id}`
        );
    }
  )
