...
servicesRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Entity.find({ category: { $elemMatch: { $eq: 'isServices' } } })
    ...
    (req, res, next) => {
      Entity.create(req.body)
        .then((entity) => {
          ...
          // res <-- no longer used
          //   .status(403)
          //   .end(`${req.method} operations are not supported on ./services path name.`);
        })
        .catch((err) => next(err));
    }
  )
    ...
    (req, res, next) => {
      Entity.deleteMany()
        .then((response) => {
          ...
          // res <-- no longer used
          //   .status(403)
          //   .end(`${req.method} opterations are not supported on ./services path name.`);
        })
        .catch((err) => next(err));
    }
  );
  ...
  .post(
    ...
    (req, res) => {
      ...
      res
        .status(403)
        .end(
          `${req.method} operations are not supported on ./services path name with a parameter of /${req.params.entity_id}.`
        );
    }
  )
  .put(
    ...
    (req, res, next) => {
      Entity.findByIdAndUpdate(
        ...
        .then((entity) => {
          ...
          //   .status(403)
          //   .end(
          //     `${req.method} opterations are not supported on ./services path name with a parameter of /${req.params.entity_id}.`
          //   );
        })
        .catch((err) => next(err));
    }
  ...
  .put(
    ...
    (req, res) => {
      res
        .status(403)
        .end(
          `${req.method} opteration not supported on /services/${req.params.entity_id}/comments`
        );
    }
  )
  .delete(
  ...
  .post(
    ...
    (req, res) => {
      res
        .status(403)
        .end(
          `${req.method} opteration not supported on /services/${req.params.entity_id}/comments/${req.params.comment_id}`
        );
    }
  )
